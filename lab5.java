...
public class lab5 {
    ...
    public static boolean getPrediction() {
		int GHRindex = getPredictionArrayIndex();
		if (predictions[GHRindex] <= 1) {
			return false;
		} else {
			return true;
		}
    ...
    public static int getPredictionArrayIndex() {
        int index = 0;
        for (int i = 0; i < globalHistoryRegister.length; i++) {
            index = index << 1; // Shift index left by 1 (same as multiplying by 2)
            index = index + globalHistoryRegister[i]; // Add the current bit from GHR to index
        }
        return index;
    }
    

    public static void updatePrediction(boolean branchTaken) {		
		int GHRindex = getPredictionArrayIndex();
		if (branchTaken){
			if(predictions[GHRindex] < 3) {
				predictions[GHRindex]++;
			}
		} else {
			if(predictions[GHRindex] > 0) {
				predictions[GHRindex]--;
			}
		}
		for (int i=globalHistoryRegister.length-1; i>0; i--){
			globalHistoryRegister[i] = globalHistoryRegister[i-1];
		}
		if (branchTaken) {
			globalHistoryRegister[0] = 1;
		} else {
			globalHistoryRegister[0] = 0;
		}
		return;
	}
        ...
        while (!command.equals("q")) {
            ...
            switch (tokens[0]) {
                case "h":                    
                    ...
                    break;
                case "b":
                    double percentCorrect = 100.0*((double)lab5.correctPreds/lab5.numPreds);
                    System.out.println("accuracy " + String.format("%.2f", percentCorrect) + "% (" + lab5.correctPreds + " correct predictions, " + lab5.numPreds + " predictions)");
                    break;
                case "p":
                    printPipelineState();
                    break;
                    ...
                    while (pc < instructionsList.size()) {
                        ...
                        if(desiredPcLocation >= 0) {
                            ...
                            if(pc == desiredPcLocation) {
                                pc = desiredPcLocation;
                                desiredPcLocation = -1;
                                //if(!wasNoneJumpSquash) {
                                    totalCycles++;
                                //    wasNoneJumpSquash = false;
                                //}
                            }
                        } else {
    ...
    static public void processPipelineInstruction(Instruction nextInstruction, Boolean inSquash) {
        ...
        if (stall) {            
            ...
            if(ifStage.getOperationString().equals("j")) {
                totalCycles--; 
            } else if(ifStage.getOperationString().equals("jr")){
                //totalCycles++; 
            } else if(ifStage.getOperationString().equals("jal")) {
                //totalCycles++; 
            }
            else {
                //totalCycles++;
        ...
        } else if(exStage != null && (exStage.getOperationString().equals("j") || exStage.getOperationString().equals("jr") || exStage.getOperationString().equals("jal")) ) {
            advancePipeline();
            ...
            totalCycles++;
            totalCycles++;
            wasNoneJumpSquash = true;
            advancePipeline();
            pipeline.set(0, nextInstruction);  // Load new instruction into IF stage
            pc++;  // Increment PC only if no stall
            //totalCycles++;
        }
        // }  else if(exStage != null && (exStage.getOperationString().equals("stall"))) {
        //     advancePipeline();
        //     pipeline.set(0, nextInstruction);  // Load new instruction into IF stage
        //     pc++;  // Increment PC only if no stall
        // }
        ...
        if (pc == instructionsList.size()) {
            for (int x=0; x<4; x++) {
                totalCycles++;
                pipeline.set(2, new Instruction("null"));  // Insert NOP at EX stage
                pipeline.set(1, new Instruction("null"));  // Insert NOP at EX stage
                pipeline.set(0, new Instruction("null"));  // Insert NOP at EX stage
                pipeline.set(0, new Instruction("null"));  // Insert NOP at EX stage
            }
        }

        // if(totalInstructions % 40 == 0) {
